...
`raises` is optional and goes with `expect = "raises"`. It names the exception class the refusal is supposed to be, one of `AttributeError`, `KeyError`, `NotImplementedError`, `TypeError` or `ValueError`, and naming one alongside `expect = "differs"` is refused because those are two different claims about the same call. It exists because `expect = "raises"` on its own is satisfied by any failure at all, including a failure that has nothing to do with the divergence. The forty cases asserting `engine/inplace` all take a copy of a frame, run the mutating call on the copy and hand the copy back, so a method firepanda has never written raises an `AttributeError` from inside the case rather than a refusal from the library, and before this field existed twenty of them ticked the divergence on the strength of `'DataFrame' object has no attribute 'clip'`. With the class named, a refusal the entry did not describe fails the run instead of passing for it, and the message says either the library changed how it refuses or the case is reaching something the entry is not about. The list of classes is short on purpose: a refusal a caller is meant to catch has to be a class they would think to write.

An entry that names a class is still not matched by the harness's own signal that the driver has no entry for a case. That signal is raised by this package rather than by the library under test, so there is nothing for a statement about the library to be true of, and letting `NotImplementedError` match it would tick a divergence on the strength of a harness message.
...
**`inplace=`.** Raises `NotImplementedError` naming the parameter and saying that the answer is a new frame over shared Arrow buffers. Forty two pandas callables take the parameter and the list of them is read out of the surface inventory rather than written by hand, which is the pattern for any divergence that touches a parameter rather than a name. Thirty eight of the forty two are refused. The other four are `rename` and `set_names` on `Index` and on `DatetimeIndex`, where firepanda honours `inplace` instead, because a level name is not data and changing one does not move a single label, which is the same reason pandas treats an index as mutable in that one respect. Those four have ordinary cases beside the other index cases rather than entries here, and the test that every pandas callable taking the parameter is covered reads the case registry rather than the divergence block so that honouring a parameter does not look like forgetting it.
