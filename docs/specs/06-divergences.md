...
**`inplace=`.** Raises `NotImplementedError` naming the parameter and saying that the answer is a new frame over shared Arrow buffers. Forty two pandas callables take the parameter and the list of them is read out of the surface inventory rather than written by hand, which is the pattern for any divergence that touches a parameter rather than a name. Forty of the forty two are refused. The other two are `Index.rename` and `DatetimeIndex.rename`, where firepanda honours `inplace` instead, because a level name is not data and renaming one does not move a single label, which is the same reason pandas treats an index as mutable in that one respect. Those two have ordinary cases beside the other index cases rather than entries here, and the test that every pandas callable taking the parameter is covered reads the case registry rather than the divergence block so that honouring a parameter does not look like forgetting it.
