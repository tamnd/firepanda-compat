...
That rule holds up only if the case asked a question with one answer, and the third possibility is the one that cost this suite twenty documents. pandas can document an order, or document that it is undefined, or say nothing and return one anyway, and the third is what `sort_values` does inside a group of equal keys under its default unstable kind. An order pandas neither documents nor repeats is not a promise and is not comparable, so the fix is in the case rather than in the comparison: give it a total order, by naming a tiebreaking key or by naming the kind, and never by relaxing the row order rule to cover it. Document 23 is the worked example and it carries the cheap check, which is to run the case under every ordering the library offers and see whether the answer moves.
