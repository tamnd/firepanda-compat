...
Some methods have no method here to call, because they live only in firepanda's Python layer. `DataFrame.select_dtypes` matches column types against numpy's type tree, which is a pandas compatibility rule rather than a dataframe operation, so the core does not have it and a branch here would have to carry a second copy of the tree in Mojo. Those cases say `in_process=True` in their declaration, which routes them to the module instead, and they need a note saying what this program cannot reach. Document 36 is why the escape exists and how narrow it is meant to be. It is not for a branch that is merely tedious to write.
